...
export function MeetingProvider({ children }: { children: React.ReactNode }) {
  ...
  const endMeeting = async (meetingId: string, shouldNavigate = true) => {
    try {
      const mongoId = meetingId.replace('meeting-', '');
      console.log('Ending meeting with ID:', mongoId);

      // First update the meeting status
      const response = await fetch(`/api/meetings/${mongoId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: 'ended'
      ...
      // Then notify other participants
